...
/**
 * Gates the Cosy Critters mixins on the mod being loaded, so clients and servers
 * without Cosy Critters do not error on a missing target class.
 */
public class CosyCrittersMixinGate implements IMixinConfigPlugin {

    private final boolean present = FabricLoader.getInstance().isModLoaded("cosycritters");
