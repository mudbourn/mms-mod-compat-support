...
/**
 * Gates the Particle Rain biome memo on the mod being present. Particle Rain
 * is a client-side pack mod, so the target class is simply absent on the
 * server and on any client that has dropped it.
 */
public class ParticleRainMixinGate implements IMixinConfigPlugin {

    private final boolean present = FabricLoader.getInstance().isModLoaded("particlerain");
